...
# argv <- list("sex_combined_pd_cmr_cpgs.txt", "male")
num_cores <- 64L
...
# top_10_cases <- sort(colSums(env_data[, -c(1)], na.rm = T), decreasing = TRUE, index.return = TRUE)$ix
# envs <- colnames(env_data)[-c(1)] # top represented
    ...
    function(i) {
      tryCatch({
          ...
          if (!is.na(row$env)) {
            if (grepl("G", row$model)) {
              if (row$model == "GxE") { # GxE
                fit <- with(df, rlm(formula(format(update(base_formula, ~ . + G + E + GxE))), weights = weights, psi = psi.huber,maxit = 100))
              } else { # G+E
                fit <- with(df, rlm(formula(format(update(base_formula, ~ . + G + E))), weights = weights, psi = psi.huber,maxit = 100))
              }
              base_model <- with(df, rlm(formula(format(base_formula)), weights = weights, psi = psi.huber,maxit = 100))
            } else { # E only
              base_model <- with(df, rlm(formula(format(base_formula)), weights = weights, psi = psi.huber,maxit = 100))
              fit <- with(df, rlm(formula(format(update(base_formula, ~ . + E))), weights = weights, psi = psi.huber,maxit = 100))
            }
...
manifest_from_cpg <- function(cpg) {
  ...
  if (length(snps) > 0) {
    ...
    return(data.table(bind_rows(g_models, e_models, ge_models)))
  } else {
    return(data.table())
  }
}
